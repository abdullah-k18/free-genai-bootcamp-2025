...
def load(app):
  ...
  def get_words():
    try:
      ...
      sort_by = request.args.get('sort_by', 'urdu')  # Default to sorting by 'urdu'
      order = request.args.get('order', 'asc')  # Default to ascending order

      # Validate sort_by and order
      valid_columns = ['urdu', 'romaji', 'english', 'correct_count', 'wrong_count']
      ...
      cursor.execute(f'''
        SELECT w.id, w.urdu, w.romaji, w.english, 
